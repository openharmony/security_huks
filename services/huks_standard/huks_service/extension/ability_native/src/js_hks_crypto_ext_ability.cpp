    ...
    NativeReference *jsObj, InputArgsParser argParser, ResultValueParser retParser)
{
    ...
    auto task = [weakPtrParam]() {
        ...
        HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(param == nullptr || param->jsRuntime == nullptr, "failed to get CallJsParam.");

        AbilityRuntime::HandleScope handleScope(*param->jsRuntime);
        param->errcode = DoCallJsMethod(param);
        HKS_EXT_IF_TRUE_LOGE(param->errcode != HKS_SUCCESS, "failed to call DoCallJsMethod.");
...
void GetAuthUkeyPinParams(const napi_env &env, const napi_value &funcResult, CryptoResultParam &resultParams)
{
    ...
    if (status == napi_ok && napiAuthState != nullptr) {
        ...
        HKS_EXT_IF_TRUE_LOGE(status != napi_ok, "Convert js value authState failed, status:%d", status);
    } else {
...
void HksCertInfoToString(std::vector<HksCertInfo> &certInfoVec, std::string &jsonStr)
{
    ...
    HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(jsonObj.IsNull(), "jsonObj is null");
    ...
    for (auto certInfo : certInfoVec) {
        ...
        std::string certStr = "";
        HKS_EXT_IF_TRUE_LOGE_BREAK(BlobToBase64String(certInfo.certsArray, certStr) != HKS_SUCCESS,
            "blob to base64 failed");
...
void GetExportCertificateParams(const napi_env &env, const napi_value &funcResult, CryptoResultParam &resultParams)
{
    ...
    for (uint32_t i = 0; i < length; i++) {
        ...
        HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(result != napi_ok, "Convert js certInfo fail. result:%d", result);
    ...
    std::shared_ptr<AbilityRuntime::AbilityHandler> &handler, const sptr<IRemoteObject> &token)
{
    ...
    std::string srcPath = "";
    HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(abilityInfo_ == nullptr, "abilityInfo_ is nullptr");
    GetSrcPath(srcPath);
    ...
    HKS_EXT_IF_TRUE_LOGE(status == napi_ok && exception != nullptr, "Clear Error in jsRuntime_.");
    
    HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(jsObj_ == nullptr, "Failed to get jsObj_");

    HKS_EXT_IF_TRUE_LOGE_RETURN_VOID(jsObj_->GetNapiValue() == nullptr, "Failed to get JsHksCryptoExtAbility value");
}
    ...
    std::string &handle, struct HksExternalErrorInfo **errInfo)
{
    ...
    HKS_IF_TRUE_EXCU(dataParam->errInfo != nullptr && errInfo != nullptr, *errInfo = dataParam->errInfo);
