...
extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
    struct HksBlob rsaAlias = { 21, reinterpret_cast<uint8_t *>(const_cast<char *>("fuzz_certchain_rsa")) };
    ...
    printf("fuzz_getcertificatechain init: GenerateKey ret=%d\n", ret);

    struct HksCertChain certChain = { nullptr, 0 };
    WrapParamSet certPs = BuildFixedParamSet({ { .tag = HKS_TAG_ALGORITHM, .uint32Param = HKS_ALG_RSA },
        { .tag = HKS_TAG_KEY_SIZE, .uint32Param = HKS_RSA_KEY_SIZE_2048 },
        { .tag = HKS_TAG_PURPOSE, .uint32Param = HKS_KEY_PURPOSE_SIGN | HKS_KEY_PURPOSE_VERIFY },
        { .tag = HKS_TAG_DIGEST, .uint32Param = HKS_DIGEST_SHA256 },
        { .tag = HKS_TAG_PADDING, .uint32Param = HKS_PADDING_PSS } });
    ret = HksGetCertificateChain(&rsaAlias, certPs.s, &certChain);
    printf("fuzz_getcertificatechain init: HksGetCertificateChain ret=%d\n", ret);
    return 0;
}
