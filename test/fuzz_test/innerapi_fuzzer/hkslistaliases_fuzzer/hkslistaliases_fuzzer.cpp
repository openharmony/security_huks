...
int32_t DoSomethingInterestingWithMyAPI(FuzzedDataProvider &fdp)
{
    ...
    int32_t ret = HksListAliases(ps.s, &keyAliasSet);
...
extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
    struct HksBlob rsaAlias = { 17, reinterpret_cast<uint8_t *>(const_cast<char *>("fuzz_list_rsa")) };
    ...
    printf("fuzz_listaliases init: GenerateKey ret=%d\n", ret);

    struct HksKeyAliasSet *aliasSet = nullptr;
    WrapParamSet listPs = BuildFixedParamSet({});
    ret = HksListAliases(listPs.s, &aliasSet);
    printf("fuzz_listaliases init: HksListAliases ret=%d\n", ret);
    return 0;
}
