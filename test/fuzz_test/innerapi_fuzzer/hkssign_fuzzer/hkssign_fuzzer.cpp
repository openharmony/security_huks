...
int32_t DoSomethingInterestingWithMyAPI(FuzzedDataProvider &fdp)
{
    ...
    std::vector<uint8_t> sigBuf(512);
    struct HksBlob signature = { static_cast<uint32_t>(sigBuf.size()), sigBuf.data() };

    WrapParamSet ps = ConstructParamSetFromFdp(fdp);

    return HksSign(&keyAlias, ps.s, &srcData, &signature);
}
}}}

extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
    struct HksBlob rsaAlias = { 14, reinterpret_cast<uint8_t *>(const_cast<char *>("fuzz_sign_rsa")) };
    ...
    printf("fuzz_sign init: GenerateKey ret=%d\n", ret);
    ...
    printf("fuzz_sign init: HksSign ret=%d\n", ret);
    return 0;
}
