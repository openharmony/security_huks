 ...
 */
#include "hksanonattestkey_fuzzer.h"
...
extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
    struct HksBlob rsaAlias = { 21, reinterpret_cast<uint8_t *>(const_cast<char *>("fuzz_anonattest_rsa")) };
    ...
    printf("fuzz_anonattestkey init: GenerateKey ret=%d\n", ret);
    ...
    WrapParamSet attestPs = BuildFixedParamSet({ { .tag = HKS_TAG_ALGORITHM, .uint32Param = HKS_ALG_RSA },
        ...
        { .tag = HKS_TAG_DIGEST, .uint32Param = HKS_DIGEST_SHA256 },
        { .tag = HKS_TAG_PADDING, .uint32Param = HKS_PADDING_PSS } });
    ret = HksAnonAttestKey(&rsaAlias, attestPs.s, &certChain);
    printf("fuzz_anonattestkey init: HksAnonAttestKey ret=%d\n", ret);
    return 0;
}
