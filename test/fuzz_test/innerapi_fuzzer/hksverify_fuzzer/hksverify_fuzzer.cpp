...
{
    if (data == nullptr || size < (BLOB_NUM * sizeof(uint64_t))) {
        return -1;
    }
    ...
    struct HksBlob signature = { sizeof(uint32_t), ReadData<uint8_t *>(data, size, sizeof(uint32_t)) };

    WrapParamSet ps = ConstructHksParamSetFromFuzz(data, size);

    [[maybe_unused]] int ret = HksVerify(&key, ps.s, &srcData, &signature);
