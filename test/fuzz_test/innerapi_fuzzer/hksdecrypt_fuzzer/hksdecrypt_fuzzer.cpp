...
int32_t DoSomethingInterestingWithMyAPI(FuzzedDataProvider &fdp)
{
    ...
    uint32_t ptSize = fdp.ConsumeIntegralInRange<uint32_t>(1, 512);
...
extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
    struct HksBlob aesAlias = { 16, reinterpret_cast<uint8_t *>(const_cast<char *>("fuzz_decrypt_aes")) };
    ...
    printf("fuzz_decrypt init: GenerateKey ret=%d\n", ret);
    ...
    printf("fuzz_decrypt init: Encrypt ret=%d\n", ret);

    uint8_t plainBuf[512] = {0};
    struct HksBlob plainText = { 512, plainBuf };
    WrapParamSet decPs = BuildFixedParamSet({ { .tag = HKS_TAG_ALGORITHM, .uint32Param = HKS_ALG_AES },
        { .tag = HKS_TAG_PURPOSE, .uint32Param = HKS_KEY_PURPOSE_DECRYPT },
        { .tag = HKS_TAG_BLOCK_MODE, .uint32Param = HKS_MODE_CBC },
        { .tag = HKS_TAG_PADDING, .uint32Param = HKS_PADDING_PKCS7 },
        { .tag = HKS_TAG_IS_KEY_ALIAS, .boolParam = true } });
    ret = HksDecrypt(&aesAlias, decPs.s, &cipherText, &plainText);
    printf("fuzz_decrypt init: HksDecrypt ret=%d\n", ret);
    return 0;
}
