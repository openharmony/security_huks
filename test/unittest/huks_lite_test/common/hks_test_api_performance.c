    ...
    struct HksParamSet *paramSetOut, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        int32_t ret = HksGenerateKeyForDe(keyAlias, paramSetIn, paramSetOut);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    const struct HksBlob *plainText, struct HksBlob *cipherText, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksEncryptForDe(key, paramSet, plainText, cipherText);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    const struct HksBlob *cipherText, struct HksBlob *plainText, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksDecryptForDe(key, paramSet, cipherText, plainText);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    const struct HksBlob *srcData, struct HksBlob *mac, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksMacForDe(key, paramSet, srcData, mac);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    struct HksBlob *derivedKey, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksDeriveKeyForDe(paramSet, masterKey, derivedKey);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
...
{
    for (uint32_t i = 0; i < performTimes; ++i) {
        int32_t ret = HksKeyExistForDe(keyAlias, NULL);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    const struct HksBlob *peerPublicKey, struct HksBlob *agreedKey, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksAgreeKeyForDe(paramSet, privateKey, peerPublicKey, agreedKey);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
    ...
    const struct HksBlob *srcData, struct HksBlob *hash, uint32_t performTimes)
{
    ...
    for (uint32_t i = 0; i < performTimes; ++i) {
        ...
        int32_t ret = HksHash(paramSet, srcData, hash);
        if (ret != HKS_SUCCESS) {
            return ret;
        }
    }
