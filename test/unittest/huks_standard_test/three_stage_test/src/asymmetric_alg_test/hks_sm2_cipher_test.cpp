    ...
    const struct HksBlob *inData)
{
    ...
    do {
        ...
        EXPECT_EQ(ret, HKS_SUCCESS) << "HksSm2CipherTestEncrypt failed.";
        if (ret != HKS_SUCCESS) {
            ret = ((ret == param.encrypt.result) ? HKS_SUCCESS : ret);
...
HWTEST_F(HksSm2CipherTest, HksSm2CipherTest001, TestSize.Level0)
{
    ...
    GenEncryptDecryptParam param { POSITIVE_CASE_GEN_PARAM, POSITIVE_CASE_ENCRYPT_PARAM, POSITIVE_CASE_DECRYPT_PARAM };
    int ret = HksSm2CipherTestRun(&keyAlias, param, &inData);
...
HWTEST_F(HksSm2CipherTest, HksSm2CipherTest002, TestSize.Level0)
{
    ...
    for (const struct TestCaseParam &negativeGenParam : NEGATIVE_CASE_GEN_PARAMS) {
        GenEncryptDecryptParam param { negativeGenParam, POSITIVE_CASE_ENCRYPT_PARAM, POSITIVE_CASE_DECRYPT_PARAM };
        ret = HksSm2CipherTestRun(&keyAlias, param, &inData);
    ...
    for (const struct TestCaseParam &negativeEncryptParam : NEGATIVE_CASE_ENCRYPT_PARAMS) {
        GenEncryptDecryptParam param { POSITIVE_CASE_GEN_PARAM, negativeEncryptParam, POSITIVE_CASE_DECRYPT_PARAM };
        ret = HksSm2CipherTestRun(&keyAlias, param, &inData);
    ...
    for (const struct TestCaseParam &negativeDecryptParam : NEGATIVE_CASE_DECRYPT_PARAMS) {
        GenEncryptDecryptParam param { POSITIVE_CASE_GEN_PARAM, POSITIVE_CASE_ENCRYPT_PARAM, negativeDecryptParam };
        ret = HksSm2CipherTestRun(&keyAlias, param, &inData);
...
HWTEST_F(HksSm2CipherTest, HksSm2CipherTest003, TestSize.Level0)
{
    ...
    GenEncryptDecryptParam param { NON_DIGEST_CASE_GEN_PARAM, NON_DIGEST_CASE_ENCRYPT_PARAM, NON_DIGEST_CASE_DECRYPT_PARAM };
    int ret = HksSm2CipherTestRun(&keyAlias, param, &inData);
...
HWTEST_F(HksSm2CipherTest, HksSm2CipherTest004, TestSize.Level0)
{
    ...
    struct HksBlob keyAlias = { strlen(keyAliasString), (uint8_t *)keyAliasString };
    for (int16_t i = 0; i < inDataArrSize; i++) {
