    ...
    const uint32_t paramCnt, struct HksParamSet *genParamSet)
{
    ...
    return HksGenerateKeyForDe(keyAlias, genParamSet, nullptr);
}
    ...
    struct HksParamSet *secondParamSet, struct HksBlob *newKeyAlias)
{
    ...
    if (HksExportPublicKeyForDe(keyAlias, firstParamSet, &publicKey) != HKS_SUCCESS) {
        return HKS_FAILURE;
    }
    return HksImportKeyForDe(newKeyAlias, secondParamSet, &publicKey);
}
    ...
    const struct HksParamSet *encryptParamSet, const struct HksBlob *inData, struct HksBlob *cipherText)
{
    ...
    int32_t ret = HksInitForDe(keyAlias, encryptParamSet, &handle, nullptr);
    if (ret != HKS_SUCCESS) {
    ...
    const struct HksParamSet *decryptParamSet, const struct HksBlob *cipherTest, struct HksBlob *plainText)
{
    uint8_t tmpHandle[sizeof(uint64_t)] = {0};
    struct HksBlob handle = { sizeof(uint64_t), tmpHandle };
    int32_t ret = HksInitForDe(keyAlias, decryptParamSet, &handle, nullptr);
    if (ret != HKS_SUCCESS) {
    ...
    const struct HksParamSet *signVerifyParamSet, const struct HksBlob *inData, struct HksBlob *signedData)
{
    uint8_t tmpHandle[sizeof(uint64_t)] = {0};
    struct HksBlob handle = { sizeof(uint64_t), tmpHandle };
    int32_t ret = HksInitForDe(keyAlias, signVerifyParamSet, &handle, nullptr);
    if (ret != HKS_SUCCESS) {
...
int32_t HksCrossTestHmac(const struct HksBlob *keyAlias, const struct HksParamSet* hmacParamSet)
{
    ...
    int32_t ret = HksMacForDe(keyAlias, hmacParamSet, &inData, &mac);
    if (ret != HKS_SUCCESS) {
    ...
    const uint32_t genParamsCnt)
{
    ...
    ret = HksGenerateKeyForDe(keyAlias01, genParamSet, nullptr);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksGenerateKey01 failed.");
        return HKS_FAILURE;
    }

    ret = HksGenerateKeyForDe(keyAlias02, genParamSet, nullptr);
    if (ret != HKS_SUCCESS) {
...
{
    int32_t ret = HksExportPublicKeyForDe(keyAlias01, genParamSet, publicKey01);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksExportPublicKey02 failed.");
        return HKS_FAILURE;
    }

    ret = HksExportPublicKeyForDe(keyAlias02, genParamSet, publicKey02);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksExportPublicKey02 failed.");
        return HKS_FAILURE;
    }
...
{
    HksDeleteKeyForDe(keyAlias1, nullptr);
    HksDeleteKeyForDe(keyAlias2, nullptr);
}
    ...
    struct HksParamSet *initParamSet, struct HksParamSet *finishParamSet)
{
    ...
    ret = HksInitForDe(keyAlias, initParamSet, &handleDerive, nullptr);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksInit failed.");
        return HKS_FAILURE;
    }
    ...
    ret = HksUpdateForDe(&handleDerive, initParamSet, &inData, &outDataU);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksUpdate failed.");
        return HKS_FAILURE;
    }
    ...
    ret = HksFinishForDe(&handleDerive, finishParamSet, &inData, &outData);
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("HksFinish failed.");
        return HKS_FAILURE;
    }
    ...
    ret = HksDeriveKeyForDe(initParamSet, keyAlias, &derivedKey);
    if (ret != HKS_SUCCESS) {
