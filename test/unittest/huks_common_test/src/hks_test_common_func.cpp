    ...
    *blob = (struct HksBlob *)HksTestMalloc(sizeof(struct HksBlob));
    ...
    if (blobDataExist) {
        ...
        for (uint32_t i = 0; i < (realBlobDataSize - 1) / HKS_TEST_1024; i++) {
            ...
            EXPECT_TRUE(HksGenerateRandom(NULL, &tmp) == 0);

            EXPECT_TRUE(memcpy_s((*blob)->data + offset, realBlobDataSize - offset, tmp.data, tmp.size) == EOK);
            offset += tmp.size;
        ...
        HksInitialize();
        EXPECT_TRUE(HksGenerateRandom(NULL, &tmp) == 0);
        EXPECT_TRUE(memcpy_s((*blob)->data + offset, remainSize, tmp.data, tmp.size) == EOK);
        HksTestFree(tmp.data);
    } else {
        (*blob)->data = NULL;
    }
...
int32_t TestGenerateKeyParamSetPre(struct GenerateKeyParamSetStructure *paramStruct,
...
int32_t TestGenerateKeyParamSetPost(struct GenerateKeyParamSetStructure *paramStruct,
    struct HksParamSet *paramSet)
{
    if (!paramStruct->paramSetExist) {
        return 0; // do nothing, return null
