...
namespace OHOS {
namespace ACELite {
static int32_t HksCallAbortSession(struct HksBlob *handle, struct HksParamSet *paramSet)
{
    return HksAbort(handle, paramSet);
}

JSIValue HksLiteModule::abortSession(const JSIValue thisVal, const JSIValue* args, uint8_t argsNum)
{
    JSIValue undefValue = JSI::CreateUndefined();
    uint32_t minNumArgs = 3;
    if (argsNum < minNumArgs || args == nullptr) {
        HKS_LOG_E("abortSession args invalid, args num(%{public}d).", argsNum);
        return undefValue;
    }
    ...
    do {
        ...
        ret = HksCallAbortSession(&handle, paramSet);
    } while (0);
    if (ret == HKS_SUCCESS) {
