...
JSIValue HksLiteModule::getKeyProperties(const JSIValue thisVal, const JSIValue* args, uint8_t argsNum)
{
    (void)thisVal;
    (void)args;
    (void)argsNum;
    ...
    if (argsNum < minNumArgs || args == nullptr) {
        HKS_LOG_E("getKeyProperties args invalid, args num(%{public}d).", argsNum);
        return undefValue;
    }
