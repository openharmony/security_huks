...
namespace OHOS {
namespace ACELite {
static int32_t HksCallInitSession(const struct HksBlob *aliasBlob, struct HksParamSet *paramSet,
    struct HksBlob *handle)
{
    handle->data = static_cast<uint8_t *>(HksMalloc(HKS_HANDLE_SIZE));
    if (handle->data == nullptr) {
        HKS_LOG_E("malloc handle data failed");
        return HKS_ERROR_MALLOC_FAIL;
    }
    handle->size = HKS_HANDLE_SIZE;
    int32_t ret = InitHuksModule();
    if (ret != HKS_SUCCESS) {
        HKS_LOG_E("init huks failed");
        return ret;
    }
    ret =  HksInit(aliasBlob, paramSet, handle, nullptr);
    return ret;
}

JSIValue HksLiteModule::initSession(const JSIValue thisVal, const JSIValue* args, uint8_t argsNum)
{
    JSIValue undefValue = JSI::CreateUndefined();
    uint32_t minNumArgs = 3;
    if (argsNum < minNumArgs || args == nullptr) {
        HKS_LOG_E("initSession args invalid, args num(%{public}d).", argsNum);
        return undefValue;
    }
    ...
    do {
        ...
        ret = HksCallInitSession(&aliasBlob, paramSet, &handle);
    } while (0);
    if (ret == HKS_SUCCESS) {
        struct HksLiteApiResult result = { nullptr, &handle, false };
