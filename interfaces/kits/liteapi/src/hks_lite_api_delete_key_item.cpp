...
namespace OHOS {
namespace ACELite {
static int32_t HksCallDeleteKey(const struct HksBlob *aliasBlob, struct HksParamSet *paramSet)
{
    ...
    ret = HksDeleteKey(aliasBlob, paramSet);
    return ret;
}

JSIValue HksLiteModule::deleteKeyItem(const JSIValue thisVal, const JSIValue* args, uint8_t argsNum)
{
    JSIValue undefValue = JSI::CreateUndefined();
    uint32_t minNumArgs = 3;
    if (argsNum < minNumArgs || args == nullptr) {
        HKS_LOG_E("deleteKeyItem args invalid, args num(%{public}d).", argsNum);
        return undefValue;
    }
    ...
    do {
        ...
        ret = HksCallDeleteKey(&aliasBlob, paramSet);
    } while (0);
    if (ret == HKS_SUCCESS) {
