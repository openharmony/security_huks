...
{
    int32_t ret = HksAuthUkeyPin((const struct HksBlob *) index,
...
{
    int32_t ret = HksGetUkeyPinAuthState((const struct HksBlob *) index, (const struct HksParamSet *) paramSetIn, stateOut);
    return ConvertApiResult(ret);
}
