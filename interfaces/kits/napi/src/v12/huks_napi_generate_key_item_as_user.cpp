...
constexpr int HUKS_NAPI_GENERATE_KEY_AS_USER_ARGS_COUNT = 3;
static napi_value GenerateKeyAsUserParseParams(napi_env env, napi_callback_info info, GenerateKeyAsyncContext context)
{
    size_t argc = HUKS_NAPI_GENERATE_KEY_AS_USER_ARGS_COUNT;
    napi_value argv[HUKS_NAPI_GENERATE_KEY_AS_USER_ARGS_COUNT] = { 0 };
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));

    if (argc != HUKS_NAPI_GENERATE_KEY_AS_USER_ARGS_COUNT) {
    ...
    if (result == nullptr) {
        ...
        HKS_LOG_E("GenerateKeyAsUserParseParams could not get user id value");
        return nullptr;
    }
    index++;
...
napi_value HuksNapiItemGenerateKeyAsUser(napi_env env, napi_callback_info info)
{
    ...
    napi_value result = GenerateKeyAsUserParseParams(env, info, context);
    if (result == nullptr) {
