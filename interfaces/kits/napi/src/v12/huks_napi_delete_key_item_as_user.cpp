...
constexpr int HUKS_NAPI_DELETE_KEY_AS_USER_ARGS_COUNT = 3;
static napi_value DeleteKeyAsUserParseParams(napi_env env, napi_callback_info info, DeleteKeyAsyncContext context)
{
    size_t argc = HUKS_NAPI_DELETE_KEY_AS_USER_ARGS_COUNT;
    napi_value argv[HUKS_NAPI_DELETE_KEY_AS_USER_ARGS_COUNT] = { 0 };
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));

    if (argc != HUKS_NAPI_DELETE_KEY_AS_USER_ARGS_COUNT) {
    ...
    if (result == nullptr) {
        ...
        HKS_LOG_E("DeleteKeyAsUserParseParams could not get user id value");
        return nullptr;
    }
    index++;
...
napi_value HuksNapiDeleteKeyItemAsUser(napi_env env, napi_callback_info info)
{
    ...
    napi_value result = DeleteKeyAsUserParseParams(env, info, context);
    if (result == nullptr) {
