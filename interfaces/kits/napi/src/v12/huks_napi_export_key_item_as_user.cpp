...
constexpr int HUKS_NAPI_EXPORT_KEY_AS_USER_ARGS_COUNT = 3;

static napi_value ExportKeyAsUserParseParams(napi_env env, napi_callback_info info, ExportKeyAsyncContext context)
{
    size_t argc = HUKS_NAPI_EXPORT_KEY_AS_USER_ARGS_COUNT;
    napi_value argv[HUKS_NAPI_EXPORT_KEY_AS_USER_ARGS_COUNT] = { 0 };
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));

    if (argc != HUKS_NAPI_EXPORT_KEY_AS_USER_ARGS_COUNT) {
    ...
    if (result == nullptr) {
        ...
        HKS_LOG_E("ExportKeyAsUserParseParams could not get user id value");
        return nullptr;
    }
    index++;
...
napi_value HuksNapiExportKeyItemAsUser(napi_env env, napi_callback_info info)
{
    ...
    napi_value result = ExportKeyAsUserParseParams(env, info, context);
    if (result == nullptr) {
